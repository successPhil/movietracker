...
docker build --build-arg VITE_BASE_URL=$BASE_URL -t $DOCKERHUB_UNAME/movie-app_webserver-prod:$NEW_VERSION -f webserver/Dockerfile . --no-cache
docker push $DOCKERHUB_UNAME/movie-app_webserver-prod:$NEW_VERSION
...
docker build -t $DOCKERHUB_UNAME/movie-app_api-prod:$NEW_VERSION -f backend/Dockerfile ./backend --no-cache
docker push $DOCKERHUB_UNAME/movie-app_api-prod:$NEW_VERSION